...
- `demo-snapshot-fetch` / `demo-snapshot-mirror`: keychain-backed quota snapshot hydration for the current persisted `device_id`
...
- The mobile app reuses a keychain-backed `device_id` when it already exists. GP-115 no longer treats `UIDevice.identifierForVendor` or any other lookup key as authoritative recovery after uninstall/reinstall.
- Snapshot hydration remains keyed by `device_id`, so continuity only exists when the same persisted keychain identity is still available to the app.
...
7. **Seed + snapshot validation**:
   ...
   The snapshot should report `attempts_used=2` and `server_lock_reason="quota"`; that is the same-device third-attempt block whenever the app retains the same keychain-backed `device_id`.
