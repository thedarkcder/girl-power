1. Problem: Guarantee each device is limited to two Squat Coaching demos without relying on unsupported anonymous reinstall recovery while Supabase + LLM keep the gate authoritative.
...
4. Assumptions:
   ...
   - Keychain identity may survive normal app restarts and some reinstalls, but unsupported anonymous recovery paths must not be claimed or exposed.
...
5. Contract matrix:
   ...
   - attemptsUsed≥2 or decision deny/timeout/serverLock ⇒ locked(reason), route to paywall for the currently known local identity, intentional reinforcement.
6. Call-path impact scan:
   ...
   - GirlPowerApp cold start → coordinator.prepareForDemoStart(); hydrates from local persistence before showing CTA.
7. Domain term contracts:
   - demo attempt = full Squat Coaching flow; counted even if crash after logging start.
   - device_id = app-scoped keychain-backed UUID used for server logging/evaluation while the local identity remains available.
...
9. Lifecycle & state matrix:
   - States: fresh → firstAttemptActive → gatePending → secondAttemptEligible → secondAttemptActive → locked(reason);
     local persistence drives the active app flow and locked is absorbing.
10. Proposed design:
   - Keep DemoQuotaStateMachine pure (already implemented) to drive side-effect intents.
   - Implement DemoQuotaCoordinator with serial async queue; inject persistence, identity, session logger, and evaluation client.
   ...
   - Remove anon-key recovery helpers; the app stays keychain-first and server-side recovery remains unavailable until a trusted identity design exists.
...
11. Patterns used:
   ...
   - Keychain-first identity keeps the client contract honest without anonymous privileged recovery.
12. Patterns not used:
   - No timer-based polling or sleeps; evaluation waits on single request with timeout-based denial.
13. Change surface:
   - GirlPower/DemoQuota: Coordinator, repository, device identity, evaluation client, and session logging services.
...
14. Load shape & query plan:
   - Each attempt issues ≤3 HTTP calls (start log, completion log, evaluate-session) keyed by device_id indexes.
...
16. Operational integrity:
   - Rollback by clearing local persistence and relying on the existing two-call server contract; no irreversible migrations.
   - Dependencies: evaluate-session POST JSON, 3s timeout, no retries; session logger POST with one retry/backoff; concurrency serialized via coordinator queue.
17. Tests:
   ...
   - Invariant 3 ⇒ Test preserved quota state when completion returns no replacement and trusted server responses decode duplicate/rate-limit bodies deterministically.
