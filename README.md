...
This repo hosts the Girl Power iOS app plus supporting Supabase Edge Functions. Use the instructions below to configure the demo-quota workflow locally.
...
| `scripts/serve-evaluate-session.sh` | Serves `evaluate-session` with hot reload using `supabase/functions/.env.local`. Start the companion `demo-session-log` and `demo-snapshot-*` functions with `supabase functions serve ... --env-file supabase/functions/.env.local`. |

Demo quota continuity now depends on reusing the app's persisted keychain-backed `device_id`. The repo no longer advertises a lookup-key reinstall recovery path.
