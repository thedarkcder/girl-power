...
update public.demo_attempts
set request_payload = jsonb_set(
  coalesce(request_payload, '{}'::jsonb),
  '{metadata}',
  coalesce(request_payload -> 'metadata', '{}'::jsonb) || jsonb_build_object(
    'anon_session_id',
    coalesce(
      request_payload -> 'input' -> 'context' ->> 'anon_session_id',
      request_payload -> 'context' ->> 'anon_session_id',
      request_payload ->> 'anon_session_id'
    )
  ),
  true
)
where public.demo_attempts.request_payload -> 'metadata' ->> 'anon_session_id' is null
  and coalesce(
    public.demo_attempts.request_payload -> 'input' -> 'context' ->> 'anon_session_id',
    public.demo_attempts.request_payload -> 'context' ->> 'anon_session_id',
    public.demo_attempts.request_payload ->> 'anon_session_id'
  ) is not null;
