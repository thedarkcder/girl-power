...
create unique index if not exists device_links_device_unique_idx
  on public.device_links(device_id);
...
begin
  ...
  select
    device_links.user_id,
    device_links.linked_at
  into
    v_existing_user_id,
    v_linked_at
  from public.device_links
  where device_links.device_id = p_device_id
  limit 1;

  if v_existing_user_id is not null and v_existing_user_id <> p_auth_user_id then
    return query
    select
      'relink_rejected',
      null::integer,
